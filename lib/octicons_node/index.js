...
for (const key of Object.keys(data)) {
  // Temporarily remove icons without 16px SVGs until we update the library
  // to handle mupltiple SVGs per icon.
  if (!(DEFAULT_HEIGHT in data[key].heights)) {
    delete data[key]
    continue
  }
  ...
  data[key].symbol = key
  data[key].width = data[key].heights[DEFAULT_HEIGHT].width
  data[key].height = parseInt(DEFAULT_HEIGHT)
  ...
  data[key].options = {
    ...
    viewBox: `0 0 ${data[key].heights[DEFAULT_HEIGHT].width} ${DEFAULT_HEIGHT}`,
  ...
  data[key].toSVG = function(options) {
    return `<svg ${htmlAttributes(data[key], options)}>${data[key].heights[DEFAULT_HEIGHT].path}</svg>`
  }
}

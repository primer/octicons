...
test('emits one CJS module per icon', () => {
  const alert = fs.readFileSync(path.join(iconsDir, 'AlertIcon.js'), 'utf8')
  // The CJS module must export the named icon via exports.
  expect(alert).toContain('forwardRef')
  expect(alert).toContain('renderOcticon(')
  expect(alert).toContain('AlertIcon')
  // Verify it is actually CJS (uses exports assignment, not ESM syntax).
  expect(alert).not.toContain('export {')
  expect(alert).not.toContain('export const')
  // react must remain external (not inlined).
  expect(alert).toContain("require('react')")
})
...
test('package.json exports expose the "." barrel and a per-icon "./*" subpath', () => {
  ...
  expect(pkg.exports['./*'].require).toBe('./dist/icons/*.js')
  expect(pkg.exports['./*'].types).toEqual({
    import: './dist/icons/*.d.ts',
    require: './dist/icons/*.d.ts'
  })
})

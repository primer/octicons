...
test('the barrel is a pure re-export of the per-icon modules', () => {
  const barrel = fs.readFileSync(path.join(packageRoot, 'dist', 'index.js'), 'utf8')
  expect(barrel).toContain("export { AlertIcon } from './icons/AlertIcon.js'")
...
test('package.json exports expose the "." barrel and a per-icon "./*" subpath', () => {
  ...
  expect(pkg.exports['./*'].import).toBe('./dist/icons/*.js')

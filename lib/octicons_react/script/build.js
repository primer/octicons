...
const {default: generate} = generatorPackage
const __dirname = fileURLToPath(new URL('.', import.meta.url))
const octicons = JSON.parse(readFileSync(new URL('../../build/data.json', import.meta.url), 'utf8'))
  ...
  .map(([key, octicon]) => {
    ...
    const program = t.program([
      ...
      t.importDeclaration(
        ...
        t.stringLiteral('../../renderOcticon.js')
      ),
...
${icons.map(({name}) => `export {${name}} from './${name}.js'`).join('\n')}
`

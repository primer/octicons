...
import { ${icon.name} } from './dist/icons';
...
fse
  ...
  .then(() => {
    return fse
      .readFile(umdSrc, 'utf8')
      .then(content => content.replace(/.\/__generated__\//g, './'))
      .then(content => fse.writeFile(umdDest, content, 'utf8'))
  })
  .then(() => {
    return fse

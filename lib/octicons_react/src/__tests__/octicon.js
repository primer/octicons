...
describe('An icon component', () => {
  ...
  it('respects `aria-hidden="false"` if no label is present', () => {
    const {container} = render(<AlertIcon aria-hidden="false" />)
    expect(container.querySelector('svg')).toHaveAttribute('aria-hidden', 'false')
  })

  it('respects `aria-hidden="true"` if a label is present', () => {
    const {container} = render(<AlertIcon aria-hidden="true" aria-label="Alert" />)
    expect(container.querySelector('svg')).toHaveAttribute('aria-hidden', 'true')
  })
